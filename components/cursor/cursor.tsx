  ...
  return (
    <div
      ...
      style={{
        ...
        translate(${Math.sqrt(scale) * (-(Math.sqrt(Math.pow(previousVelocity.current.x, 2) + Math.pow(previousVelocity.current.y, 2))))}px)
        scaleX(${scale * (1 + (Math.sqrt(Math.pow(previousVelocity.current.x, 2) + Math.pow(previousVelocity.current.y, 2)))/16)})
        scaleY(${scale * (Math.pow(1/(1+(Math.sqrt(Math.pow(previousVelocity.current.x, 2) + Math.pow(previousVelocity.current.y, 2)))), 1/6))})
        `,
      }}
    ></div>
  );
